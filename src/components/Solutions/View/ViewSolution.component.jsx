...
const YTembed = ({ link }) => {
	return (
		<iframe
			width="560"
			height="315"
			src="https://www.youtube.com/embed/-Ur3wwV6RXw"
...
const ViewSolution = ({ id, updateFooter, updateGlobalID }) => {
	...
	const getData = async () => {
		...
		await API.graphql({
			...
			setAttachments(response.data.listAttachements.items);
		});
		await API.graphql({
			query: listFeatures,
			variables: { filter: { solutionID: { contains: id } } },
	...
	const createListObject = () => {
		let obj = {
			...
			repo: solution.repo,
			language: solution.language.toString(),
			category: solution.category.toString(),
			services: solution.services.toString(),
		};
		setListObject(obj);
	};
	React.useEffect(() => {
	...
	return (
		<div className="vs-container">
			...
			>
				<Paper
					elevation={1}
					sx={{ padding: "10px", border: "0.5px solid black" }}
				>
						...
						<Paper sx={{ padding: "3px" }} elevation={4}>
							<div className="content-heading">
								...
								<span className="content-span">
									{solution.title && solution.owner
										? `${solution.title} by ${solution.owner}`
										: "Oops! This Solution is untitled"}
								</span>
							</div>
						</Paper>
							...
							<div className="first-section">
								...
								<div className="generalization-cbox">
									<CustomBox
										...
										content={solution.generalization}
